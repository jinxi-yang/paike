...
> **文档版本**: v2.1 | **适用系统**: paike v1.1 | **更新日期**: 2026年3月8日
...
| # | 问题 | 修复方案 |
|---|------|----------|
| B1 | `adjust`接口讲师冲突误报 | 分离 Day1/Day2 检查，`combo_id`只与`combo_id`比较，`combo_id_2`只与`combo_id_2`比较 |
| B2 | `generate_schedule`课表清除不彻底 | 清除范围增加 `planning` 和 `conflict` 状态 |
| B3 | AI响应JSON解析脆弱 | 增加4种提取模式 + 尾随逗号修复 + 结构统一化 |
| B4 | 无事务回滚保护 | 已确认存在 `db.session.rollback()` |
| B5 | holidays缓存仅2026 | 改为按年份动态加载 `holidays_{year}.json` |

#### 前端修复
| # | 问题 | 修复方案 |
|---|------|----------|
| F1 | 硬编码年月 | 改为 `new Date()` 自动定位当前月 |
