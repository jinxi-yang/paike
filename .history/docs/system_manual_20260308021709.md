...
> **文档版本**: v2.0 | **适用系统**: paike v1.0 | **更新日期**: 2026年3月
...
### 9.5 节假日数据
- 系统内置了 `holidays_2026.json` 缓存文件
- 如需跨年使用，需手动更新节假日缓存或确保 timor.tech API 可用
...
| # | 问题 | 严重程度 | 位置 | 详情 |
|---|------|----------|------|------|
| 1 | **adjust接口讲师冲突误报** | 🟡 中 | `schedule.py` L286-323 | 调整课程时的讲师冲突检查使用 `or_(combo_id, combo_id_2)` 联合查询，会**交叉比对**周六讲师和周日讲师。但 `combo_id` 是周六、`combo_id_2` 是周日，属于不同天，不应交叉冲突。例如：老师A周六给A班上课、周日给B班上课，`adjust` 会误报为冲突。相比之下，`generate_schedule`（L964-988）的逻辑是正确的——它分别检查 Day1撞Day1、Day2撞Day2 |
| 2 | **课表清除策略不彻底** | 🟡 中 | `schedule.py` | `generate_schedule` 清除旧数据时只删除 `status in ('scheduled','planning')`，`conflict` 状态的记录不会被清除，可能导致数据堆积 |
| 3 | **AI响应解析脆弱** | 🟡 中 | `ai.py` | 依赖正则表达式解析 AI 返回的 JSON，格式稍有变化就会失败 |
| 4 | **无事务回滚保护** | 🟡 中 | 多处 | 批量操作（如generate_schedule）中间失败时，已写入的数据不会回滚 |
| 5 | **holidays缓存仅2026** | 🟡 中 | `schedule.py` | 本地缓存文件为 `holidays_2026.json`，跨年使用需手动更新 |

### 10.2 前端问题

| # | 问题 | 严重程度 | 详情 |
|---|------|----------|------|
| 1 | **硬编码年月** | 🟡 中 | 全局变量 `currentYear=2026, currentMonth=3`，新启动时不会自动定位当前月 |
| 2 | **课题数量硬编码** | 🟢 低 | 项目详情弹窗硬编码"共8节"文字 |
| 3 | **发布预览模板拼写错误** | 🟢 低 | `openPublishPreview` 中 `{cachedMonthData.month}` 缺少 `$` 前缀导致显示异常 |
| 4 | **顺延通知语言不一致** | 🟢 低 | `postponeConflictByWeek` 函数中 alert 消息为英文，与系统整体中文风格不符 |
| 5 | **拖拽调整警告转义异常** | 🟢 低 | 拖拽移动课程时 confirm 对话框中的 `\\n` 未正确转义为换行 |

> ✅ **已修复**: 原"新增排课"按钮（月度/周度视图中3处）已移除，因系统无独立的"新增排课"功能，排课通过创建班级或重新生成课表实现。
