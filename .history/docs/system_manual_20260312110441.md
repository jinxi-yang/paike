...
### 2.3 后端架构
- **入口**: `backend/app.py` — Flask应用初始化、蓝图注册
- **配置**: `backend/config.py` — 数据库URI、API密钥、调度参数
- **模型**: `backend/models.py` — 10个数据模型
- **路由**: `routes/` 目录下8个蓝图模块
...
**代码位置**: `backend/routes/classes.py` → `auto_schedule_class()`
...
**代码位置**: `backend/routes/schedule.py` → `generate_schedule()`
...
**代码位置**: `backend/routes/schedule.py` → `is_holiday()`
