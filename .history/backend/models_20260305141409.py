...
class Topic(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    training_type_id = db.Column(db.Integer, db.ForeignKey('training_type.id'), nullable=False, comment='项目ID')
    ...
    # 便捷属性：project_id 等价于 training_type_id
    @property
    def project_id(self):
        return self.training_type_id
    
    @project_id.setter
    def project_id(self, value):
        self.training_type_id = value
    ...
    def to_dict(self, include_combos=False):
        result = {
            'id': self.id,
            'project_id': self.training_type_id,
...
class Class(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    # project_id 在DB中仍叫 training_type_id（保持兼容），Python层面用 project 关系访问
    training_type_id = db.Column(db.Integer, db.ForeignKey('training_type.id'), nullable=False, comment='所属项目')
