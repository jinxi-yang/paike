...
def init_courses():
    ...
    courses_data = [
        # 通用课程
        {"name": "宏观经济形势与政策解读Day1", "keywords": ["宏观", "经济"], "duration_days": 1},
        {"name": "宏观经济形势与政策解读Day2", "keywords": ["宏观", "经济"], "duration_days": 1},
...
def init_combos():
    ...
    for topic in topics:
        ...
        # 仍然没有？创建默认的
        if not topic_courses:
            c1 = Course(name=f"{topic.name}-Day1", topic_id=topic.id, duration_days=1)
            c2 = Course(name=f"{topic.name}-Day2", topic_id=topic.id, duration_days=1)
