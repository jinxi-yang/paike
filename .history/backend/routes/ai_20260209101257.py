...
def extract_constraints():
    ...
    try:
        ...
        if resp.status_code != 200:
            return jsonify({'success': False, 'error': f'AI API Error: {resp.status_code} {resp.text}'}), 500
...
def get_schema():
    ...
    return jsonify({
        ...
        'response_format': {
            ...
            'constraints': {
                ...
                'fields': {
                    ...
                    'teacher_unavailable': {
                        'type': 'array',
                        'description': '教师请假/不可用信息',
                        'item_format': {
                            'teacher_name': '教师姓名（必填）',
                            'dates': '不可用日期列表，每项格式YYYY-MM-DD（必填）',
                            'reason': '原因（可选）'
                        }
                    },
                    'preferred_dates': {
        ...
        'example_extraction': {
            'input_prompt': '3月25日北京有大型医疗峰会，医疗班那天不能上课。王芳老师3月21-22日请假了。',
            'expected_output': {
                'success': True,
                'constraints': {
                    'blocked_dates': [
