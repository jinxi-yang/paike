    ...
    try:
        ...
        for cls in active_classes:
            ...
            for sat in saturdays:
                ...
                if not current_conflict_reason and teacher_name:
                    ...
                    for u in unavailable:
                        if u['teacher_name'] == teacher_name:
                            if sat_str in u['dates'] or sun_str in u['dates']:
                                current_conflict_reason = f"讲师 {teacher_name} 请假"
                                break
                
                # D. 冲突检查：同一个讲师同一天不能排两个班 
                # (注意：如果是'mark'模式，也要检查这个吗？通常物理冲突是硬冲突)
                # 暂时把“讲师撞课”也视为可标记的Soft Conflict，还是Hard？
                # 如果撞课，物理上无法分身。建议作为 Hard Conflict，或者也标记。
                # 这里暂且作为可标记冲突处理。
                if not current_conflict_reason and combo:
                    ...
                    if conflict_schedule:
                        current_conflict_reason = f"讲师撞课 ({conflict_schedule.class_.name})"
