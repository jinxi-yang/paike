...
def auto_schedule_class(class_obj):
    ...
    从班级的 start_date 开始，按间隔为每个课题分配日期，但 **仅排到当月月底**。
    - 过去的日期 → status='completed'（已经上过的课）
    - 当月的日期 → status='scheduled'（当月计划）
    - 超过当月的课题 → 不创建记录，留给「智能排课」按月推进
    ...
    for i in range(len(topics)):
        ...
        # ★ 核心变化：超过当月的课题不再排，留给智能排课
        if scheduled_date >= month_end:
            skipped = len(topics) - i
            break
        
        # 判断状态：过去的为 completed，当月的为 scheduled
