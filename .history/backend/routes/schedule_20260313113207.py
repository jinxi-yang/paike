...
from models import db, ClassSchedule, Class, MonthlyPlan, TeacherCourseCombo, Topic, ScheduleConstraint, Homeroom
import requests
...
def move_to_week():
    ...
    schedule.scheduled_date = new_date
    # 如果强制移动到有冲突的位置，标记冲突类型（不改 status）
    if warnings:
...
def _score_candidate(cls, sat, last_date, combo1, combo2, assigned_map, constraints, homeroom_overrides=None):
    ...
    if effective_homeroom_id:
        ...
        existing = ClassSchedule.query.join(Class).filter(
            Class.homeroom_id == effective_homeroom_id,
            ClassSchedule.scheduled_date == sat,
            ClassSchedule.class_id != cls.id
        ).first()
        if existing:
            # 检查existing的班级是否也有覆盖（如果该班级的homeroom已被覆盖为其他人，则不存在冲突）
            existing_effective = homeroom_overrides.get(existing.class_id, existing.class_.homeroom_id if existing.class_ else None)
...
def _run_scheduling_algorithm(year, month, constraints, conflict_mode='smart', overrides=None, skip_class_ids=None, homeroom_overrides=None, combo_overrides=None):
    """
    ...
    # Step 1: 构建候选日期池（不含上月日期，可延伸到下月第1周做弹性）
    pool_start = start_date  # 不允许排到上个月
    ...
    for info in class_infos:
        ...
        # 确定下一个未上课题：
        # - completed 的记录（任何月份）：真正上完的课
        # - scheduled 的记录（仅当月之前）：之前月份已排好的课
        # - 不看当月及之后的 scheduled（因为当月已被清除，之后的不应影响本月排课）
        ...
        combo1 = None
        combo2 = None
        user_locked_combo = False  # 用户是否手动锁定了组合
        ...
        for m in merges:
            ...
            if source_schedule:
                ...
                merged_schedule = ClassSchedule(
                    ...
                    homeroom_override_id=source_schedule.homeroom_override_id,  # 合班共用主记录的班主任
                ...
                # Bug 3 fix: 统一 notes 格式为与 merge_classes 一致的格式
                source_schedule.merge_snapshot = source_snapshot
                source_schedule.notes = f'合班主记录（含 {target_cls.name if target_cls else "?"}）'
