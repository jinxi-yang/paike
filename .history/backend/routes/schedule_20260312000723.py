...
def _run_scheduling_algorithm(year, month, constraints, conflict_mode='smart', overrides=None):
    """
...
def generate_schedule_preview():
    ...
    try:
        ...
        # 这样算法看到的 DB 状态和 Generate 一致，但不会持久化任何更改
        ...
        # 获取当月已有排课记录（已 completed/confirmed 的，供预览参考展示）
        ...
        for s in existing:
            existing_schedules.append({
                ...
                'combo2_teacher_name': s.combo_2_rel.teacher.name if s.combo_2_rel and s.combo_2_rel.teacher else None,
                'combo2_course_name': s.combo_2_rel.course.name if s.combo_2_rel and s.combo_2_rel.course else None,
            })
