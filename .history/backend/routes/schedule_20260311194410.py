...
def _get_last_class_date(class_id):
    """获取班级最后一次上课/排课的日期（跨所有月份）"""
    last = ClassSchedule.query.filter(
...
def _calculate_urgency(class_id, reference_date):
    ...
    """
    last_date = _get_last_class_date(class_id)
...
def _score_candidate(cls, sat, last_date, combo1, combo2, assigned_map, constraints):
        ...
        for a_cls_id, a_info in assigned_map.items():
            if a_info['date'] == sat and a_cls_id != cls.id:
                if a_info.get('combo1_teacher_id') == t1_id:
                    teacher_conflict = True
                    conflict_reasons.append(f'周六讲师 {combo1.teacher.name} 撞课[本轮]')
        ...
        for a_cls_id, a_info in assigned_map.items():
            if a_info['date'] == sat and a_cls_id != cls.id:
                if a_info.get('combo2_teacher_id') == t2_id:
                    teacher_conflict = True
                    conflict_reasons.append(f'周日讲师 {combo2.teacher.name} 撞课[本轮]')
    ...
    if total_on_sat >= _cfg.MAX_CLASSES_PER_SATURDAY:
        ...
        # 检查是否有同课题班级可合班（不return，继续生成合班建议）
        # 我们需要知道当前班级的 topic_id，通过 combo1 的 topic_id 获取
        if combo1:
...
def _run_scheduling_algorithm(year, month, constraints, conflict_mode='smart', overrides=None, preview_mode=False):
    """
    ...
    参数:
        ...
        preview_mode: 预览模式下，先软删除该月草稿记录再运行算法
    ...
    # 预览模式下：临时删除该月将被重排的记录，避免它们造成伪冲突
    _preview_deleted_ids = []
    if preview_mode:
        start_date_month = date(year, month, 1)
        end_date_month = date(year, month + 1, 1) if month < 12 else date(year + 1, 1, 1)
        to_exclude = ClassSchedule.query.filter(
            ClassSchedule.scheduled_date >= start_date_month,
            ClassSchedule.scheduled_date < end_date_month,
            ClassSchedule.status.in_(['scheduled', 'planning', 'conflict'])
        ).all()
        _preview_deleted_ids = [s.id for s in to_exclude]
        for s in to_exclude:
            db.session.expunge(s)  # 从 session 中移除，但不真正删除
        # 标记删除（在 session 级别临时隐藏）
        if _preview_deleted_ids:
            ClassSchedule.query.filter(
                ClassSchedule.id.in_(_preview_deleted_ids)
            ).update({ClassSchedule.status: '_preview_excluded'}, synchronize_session='fetch')
        ...
        if conflict_mode == 'smart':
            ...
            for sat in candidate_saturdays:
                ...
                if score > best_score:
                    ...
                    best_merge_suggestions = merges if is_hard else []
    ...
    # 预览模式下：恢复被临时隐藏的记录
    if preview_mode and _preview_deleted_ids:
        ClassSchedule.query.filter(
            ClassSchedule.id.in_(_preview_deleted_ids)
        ).update({ClassSchedule.status: 'scheduled'}, synchronize_session='fetch')
        # 注意：expunge 过的对象不需要恢复，因为 session.rollback 会自动处理
    ...
    try:
        ...
        for a in assignments:
            ...
            new_schedule = ClassSchedule(
                class_id=a['class_id'],
                topic_id=a['topic_id'],
                combo_id=a.get('combo_id'),
                combo_id_2=a.get('combo_id_2'),
