...
def adjust_schedule():
    ...
    # 收集需要重检的日期（调整前的日期和调整后的日期）
    dates_to_check = {schedule.scheduled_date}
    if 'date' in data and data['date']:
        old_date_str = data.get('_old_date')  # 如果前端传了旧日期
        if old_date_str:
    ...
    for check_date in dates_to_check:
        ...
        for s in same_day:
            ...
            if s.conflict_type != new_conflict:
                ...
                # 清除过时的冲突 notes
                if not new_conflict and s.notes and ('撞课' in s.notes or '手动' in s.notes or '冲突' in s.notes):
