...
def precheck_plan():
    ...
    # 兼容 project_id / training_type_id
    project_id = data.get('project_id') or data.get('training_type_id')
    ...
    topics = Topic.query.filter_by(training_type_id=project_id).order_by(Topic.sequence).all()
    if not topics:
        return jsonify({
...
def auto_schedule_class(class_obj):
    ...
    topics = list(Topic.query.filter_by(training_type_id=class_obj.training_type_id)
