...
def sync_class_statuses():
    """自动同步班级状态（每次查询时lazy执行）"""
    from datetime import date as dt_date
    today = dt_date.today()
    changed = False
    
    # planning → active: 开课日期已过
    planning_classes = Class.query.filter(
        Class.status == 'planning',
        Class.start_date != None,
        Class.start_date <= today
    ).all()
    for c in planning_classes:
        c.status = 'active'
        changed = True
    
    # active → completed: 所有课题都有 completed 的排课记录
    active_classes = Class.query.filter(Class.status == 'active').all()
    for c in active_classes:
        total_topics = Topic.query.filter_by(project_id=c.project_id).count()
        if total_topics == 0:
            continue
