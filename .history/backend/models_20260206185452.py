...
class TrainingType(db.Model):
    ...
    def to_dict(self, include_topics=False):
        ...
        if include_topics:
            result['topics'] = [t.to_dict() for t in self.topics.all()]
        return result
...
class TeacherCourseCombo(db.Model):
    ...
    # 关系
    schedules = db.relationship('ClassSchedule', backref='combo', lazy='dynamic')
...
class ClassSchedule(db.Model):
    ...
    combo_id = db.Column(db.Integer, db.ForeignKey('teacher_course_combo.id'), comment='教-课组合')
