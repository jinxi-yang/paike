    ...
    for config in mock_classes_config:
        ...
        for class_cfg in config["classes"]:
            ...
            for i, topic in enumerate(topics):
                ...
                # 获取该课题的教-课组合 (取2个不同的，用于周六和周日)
                combos = TeacherCourseCombo.query.filter_by(topic_id=topic.id)\
                                                .order_by(TeacherCourseCombo.priority.desc()).limit(2).all()
                
                combo1 = combos[0] if len(combos) > 0 else None
                combo2 = combos[1] if len(combos) > 1 else combo1  # 如果只有1个，两天用同一个
                
                schedule = ClassSchedule(
                    class_id=new_class.id,
                    topic_id=topic.id,
                    combo_id=combo1.id if combo1 else None,
                    combo_id_2=combo2.id if combo2 else None,  # 周日用不同讲师
