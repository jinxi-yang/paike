...
def _score_candidate(cls, sat, last_date, combo1, combo2, assigned_map, constraints, homeroom_overrides=None, precomputed=None):
    ...
    if last_date:
        ...
        if 25 <= interval <= 42:
            ...
            # 缩短太密集，严厉惩罚
            interval_score = max(0.0, 1.0 - (25 - interval) / 10.0)
        else: # interval > 42
            # 时间拖得越久，分数急剧下降
            interval_score = max(0.0, 1.0 - (interval - 42) / 20.0)
    else:
    ...
    weight_affinity = 0.2  # 降低亲和力绝对数值，在区间打平时（都有1.0的间隔分），这0.2分足够脱颖而出保住原日期
