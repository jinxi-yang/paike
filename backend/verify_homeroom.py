...
from models import db, Class, Homeroom, TrainingType, Topic, Teacher, Course, TeacherCourseCombo, ClassSchedule
...
def verify_homeroom_constraint():
    ...
    with app.app_context():
        ...
        tt = TrainingType.query.first()
        if not tt:
            tt = TrainingType(name="TestType")
            db.session.add(tt)
            db.session.commit()
            
        topic = Topic.query.filter_by(training_type_id=tt.id).first()
        if not topic:
            topic = Topic(training_type_id=tt.id, name="TestTopic", sequence=1)
        ...
        if not cls:
            cls = Class(training_type_id=tt.id, name=cls_name, homeroom_id=hr.id, status='active', start_date=date(2026, 4, 1))
            db.session.add(cls)
            db.session.commit()
        else:
