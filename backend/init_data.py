...
from models import db, TrainingType, Topic, Homeroom, Teacher, Course, TeacherCourseCombo, Class, ClassSchedule, Project
...
def init_mock_projects_and_classes():
    ...
    for tt in training_types:
        p = Project(name=f"{tt.name}项目", description=f"{tt.name}所属项目")
        db.session.add(p)
        db.session.flush()
        projects.append(p)
    ...
    for config in mock_classes_config:
        ...
        for class_cfg in config["classes"]:
            ...
            # assign to corresponding project for this training type
            project = projects[config["tt_idx"]] if config["tt_idx"] < len(projects) else None
            new_class = Class(
